...
export default function AllVehicleReports() {
  const { data: issues = [], isLoading } = useQuery({
    queryKey: ['all-issues'],
    queryFn: () => base44.entities.MaintenanceIssue.list('-created_date', 200)
  });
    ...
    return (
      <Card className="border-0 shadow-sm">
        <CardContent className="p-8 text-center">
          ...
          <p className="text-sm text-slate-500">Inga rapporterade fel eller skador</p>
        </CardContent>
      </Card>
    );
  }
            ...
            <CardContent className="p-4">
              ...
              <div className="flex items-center justify-between pl-12 text-xs text-slate-500">
                <div className="flex items-center gap-3">
                  ...
                  <span>Rapporterad av: {issue.created_by?.split('@')[0] || 'Okänd'}</span>
                </div>
                {issue.priority && (
