...
import { 
  ...
  Calendar, TrendingUp, Trophy, Umbrella, Users
} from "lucide-react";
import { 
  format, startOfWeek, endOfWeek, startOfMonth, endOfMonth,
  differenceInMinutes, addDays, parseISO
} from "date-fns";
...
export default function TVDashboard() {
  ...
  const employeeSummary = employees.map(emp => {
    ...
    return { name, department: emp.department || '', isActive, weekHours, monthHours };
  ...
  const activeProjects = projects.filter(p => p.status === 'pågående').map(p => {
    ...
    return { ...p, hours, weekHours, activeNow, lastActivity };
  });
  ...
  const totalWeekHours = weekEntries.filter(e => e.status !== 'rejected').reduce((s, e) => s + (e.total_hours || 0), 0);
  const totalMonthHours = monthEntries.filter(e => e.status !== 'rejected').reduce((s, e) => s + (e.total_hours || 0), 0);
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '18px' }}>
          ...
          <div>
            <div style={{ fontSize: '24px', fontWeight: '800', color: '#f1f5f9', letterSpacing: '-0.5px' }}>Realtidsdashboard</div>
